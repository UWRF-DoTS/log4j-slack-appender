...
public class SlackAppender extends AppenderSkeleton {
	...
	protected void append(LoggingEvent paramLoggingEvent) {
		...
		String message = layout.format(paramLoggingEvent); // This is just the first line message, not the stack trace;
		...
		String color = levelColors.containsKey(paramLoggingEvent.getLevel()) ? levelColors
				.get(paramLoggingEvent.getLevel()) : "good";
		String payload = "{\"channel\" : \"" + channel
				+ "\", \"username\" : \"" + user + "\", \"icon_emoji\": \""
				+ icon + "\", \"attachments\" : [{ \"text\": \"" + message
				+ "\", \"color\" : \"" + color + "\"}]}";
